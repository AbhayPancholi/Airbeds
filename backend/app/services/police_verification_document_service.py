...
def _addr_str(addr: Optional[AddressModel]) -> str:
    if not addr:
        return ""
    parts = [addr.flat_no, addr.building_no, addr.society, addr.block_sector,
             addr.street_landmark, addr.city, addr.state, addr.pin_code]
...
class PoliceVerificationDocumentService:
    ...
    async def to_pdf_bytes(self, verification_id: str) -> bytes:
        ...
        pw = A4[0] - 28 * mm  # usable page width

        # --- Styles (all use font_name so Marathi + English render correctly) ---
        base = ParagraphStyle(
            "Base", fontName=font_name, fontSize=9, leading=11,
        )
        bold = ParagraphStyle(
            "Bold", parent=base, fontName=font_name,
        )
        marathi = ParagraphStyle(
            "Marathi", parent=base, fontName=font_name, fontSize=9.5, leading=12,
        )
        title_m = ParagraphStyle(
            "TitleM", parent=marathi, alignment=1, fontSize=10, leading=13,
        )
        title_e = ParagraphStyle(
            "TitleE", parent=bold, alignment=1, fontSize=10, leading=13,
        )
        small = ParagraphStyle(
            "Small", parent=base, fontSize=7.5, leading=9,
        )
        sig_style = ParagraphStyle(
            "Sig", parent=base, fontSize=8.5, leading=10, alignment=1,
        )
        center = ParagraphStyle(
            "Center", parent=base, alignment=1,
        )
        center_bold = ParagraphStyle(
            "CenterBold", parent=bold, alignment=1,
        )
        ...
        # ============================================================
        # PHOTO ROW — 3 photo boxes with labels
        # ============================================================
        def _photo_cell(b64_data: Optional[str], width_mm: float = 22, height_mm: float = 26):
        ...
        photo_table = Table(photo_data, colWidths=[col_photo] * 3, rowHeights=[30 * mm, None])
        ...
        if owner.flats and len(owner.flats) > 0:
            flat_addr = _addr_str(owner.flats[0].address) if owner.flats[0].address else ""
        if not flat_addr:
        ...
        # The answer column spans the full right side (owner info top, tenant info below based on row)
        # ============================================================
        col_sr = 8 * mm
        col_q = 62 * mm
        ...
        rows.append([
            ...
            Paragraph(_br(
                f"<b>Name:</b>  {owner_name}" + (f"  <b>Age:</b> {owner_age}" if owner_age else ""),
        ...
        rows.append([
            ...
            Paragraph(_br(
                f"<b>Office Name:</b> {tenant_office_name}" if tenant_office_name else "",
                f"<b>Address:</b> {tenant_office_addr}" if tenant_office_addr else "",
        ...
        story.append(Paragraph(
            "घरमालक व भाडेकरांनी ,त्यांचे फोटो असलेल्या ओळखपत्राची प्रमाणित पत्र सोबत जोडावी. "
